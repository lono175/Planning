...
if __name__ == "__main__":
    ...
    agentList.append(
            FactoredQ(
                float(form['alphaLabel'].value) ,
                float(form['epsilonLabel'].value),
                float(form['gammaLabel'].value), 
                actionList,
    ...
    while 1:
        agent = agentList[0]
        env = Grid((discrete_size, discrete_size), gameScreenSize)
        state = env.start()
        ...
        while 1:
            ...
            if reward != -1:
                form['rewardLabel'].value = float(form['rewardLabel'].value) + reward
            #clock.tick(1000)
